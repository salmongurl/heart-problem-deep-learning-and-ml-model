...
export default function DiagnosticsPage() {
  ...
  const toHealthInputs = (currentInputs: UIInputs): HealthInputs => ({
    ...
    restingHeartRate: 70,
    systolicBP: currentInputs.bloodPressure,
    diastolicBP: 80,
    bmi: currentInputs.bmi,
    glucose: currentInputs.hasDiabetes ? 140 : 90,
    cholesterol: currentInputs.cholesterol,
    sleepHours: 7,
    activityMinutes: currentInputs.physicalActivity * 30,
    smoker: currentInputs.isSmoker,
    diabetic: currentInputs.hasDiabetes,
  });

  const liveMappedInputs = useMemo(() => toHealthInputs(inputs), [inputs]);
  ...
  const handlePredict = async () => {
    setLoading(true);
    try {
      const mappedInputs = liveMappedInputs;
  ...
  const LiveRiskDial = ({ value }: { value: number }) => {
    const radius = 52;
    ...
    return (
      <div className="w-full rounded-3xl border border-rose-100/60 bg-white/70 p-5 shadow-sm">
  ...
  const CircularProgress = ({
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4">
        ...
        <div className="mt-2 text-sm font-semibold text-gray-600 uppercase tracking-widest">
          {label}
        </div>
      </div>
    );
      ...
      >
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-600 font-medium max-w-lg mx-auto">
            Advanced neural networks and machine learning combined to analyze
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold text-rose-700 bg-rose-50 px-3 py-1 rounded-lg">
                      {inputs.age}{" "}
                      <span className="text-xs text-rose-900 font-normal">
                        yrs
                      </span>
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className={clsx(
                    ...
                    inputs.isSmoker
                      ? "is-active border-rose-400 bg-rose-50"
                ...
                <button
                  ...
                  className={clsx(
                    "toggle-btn flex flex-col items-center justify-center gap-2 h-24",
                    inputs.hasDiabetes
                      ? "is-active border-amber-400 bg-amber-50"
                      : "hover:bg-gray-50 text-gray-500",
                  )}
                >
                  <Activity
                    className={clsx(
                      "w-6 h-6",
                      inputs.hasDiabetes ? "text-amber-500" : "",
                    )}
                  />
                ...
                <div className="flex justify-between gap-2">
                  {[0, 1, 2, 3, 4, 5].map((val) => (
                    <button
                      key={val}
                      onClick={() =>
                        setInputs((prev: UIInputs) => ({
                          ...prev,
                          physicalActivity: val,
                        }))
                      }
                      className={clsx(
                ...
                {!isCalculated ? (
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm">
                      Adjust your vitals and run the diagnostic models to see
                      your risk profile.
                    </p>
                    <button
                  ...
                  {isCalculated && (
                    ...
                    >
                      ...
                      <div className="text-left bg-rose-50/50 p-4 rounded-xl text-sm text-rose-950 border border-rose-100">
                        ...
                        The Neural Network (Deep Learning) accounts for complex
                        non-linear interactions across your metrics, often
                        providing a more nuanced risk score than classical
                        algorithms.
                      </div>
                    </motion.div>
                  )}
